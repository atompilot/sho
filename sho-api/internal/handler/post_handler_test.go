...
func testServer(t *testing.T) (*httptest.Server, func()) {
	...
	postSvc := service.NewPostService(postStore, "test-master-password")
	h := handler.NewPostHandler(postSvc, nil, nil, nil, "test-master-password")
...
func TestUpdatePost_PasswordPolicy_NoCredential(t *testing.T) {
	srv, cleanup := testServer(t)
	defer cleanup()
...
func TestUpdatePost_MasterPassword(t *testing.T) {
	srv, cleanup := testServer(t)
	defer cleanup()

	var pub model.PublishResponse
	decodeJSON(t, postJSON(t, srv, "/api/v1/posts", map[string]any{
		"content":  "password protected for master test",
		"policy":   "password",
		"password": "regular-password",
	}), &pub)

	// Use master password to update
	updateResp := putJSON(t, srv, "/api/v1/posts/"+pub.Slug, map[string]any{
		"content":    "updated via master password",
		"credential": "test-master-password",
	})
	assert.Equal(t, http.StatusOK, updateResp.StatusCode)
}
...
func TestDeletePost_OK(t *testing.T) {
	...
	// Use master password via body credential
	delResp := func() *http.Response {
		b, _ := json.Marshal(map[string]string{"credential": "test-master-password"})
		req, _ := http.NewRequest(http.MethodDelete, srv.URL+"/api/v1/posts/"+pub.Slug, bytes.NewReader(b))
		req.Header.Set("Content-Type", "application/json")
...
func TestDeletePost_WrongCredential(t *testing.T) {
	srv, cleanup := testServer(t)
	defer cleanup()
...
func TestListPosts_OK(t *testing.T) {
	...
	// 发布两条
	postJSON(t, srv, "/api/v1/posts", map[string]any{"content": "list test 1", "policy": "password"})
